...
export default function Register() {
  ...
  const handleSubmit = async (values: RegisterPayload) => {
    ...
    try {
      ...
      const anyErr = err as { response?: { data?: { message?: string }; status?: number } };
  ...
  return (
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 420 }}>
        ...
        >
          <Title level={4} style={{
            margin: '0 0 20px', color: '#1B365D', textAlign: 'center',
            fontFamily: "'Playfair Display', Georgia, serif",
          ...
          {error && (
            <Alert message={error} type="error" showIcon style={{ marginBottom: 16, borderRadius: 6 }} />
          )}

          {/* Google sign-in — quick onboarding */}
          {googleLoading ? (
          ...
          <Divider plain style={{ color: '#aaa', fontSize: 12, margin: '16px 0' }}>
          ...
          <Form form={form} onFinish={handleSubmit} layout="vertical" requiredMark={false}>
            ...
            >
              <Input prefix={<UserOutlined style={{ color: '#C6A75E' }} />} placeholder="Nama lengkap Anda" size="large" />
            </Form.Item>
            <Form.Item
              name="email"
              label="Email"
            ...
            >
              <Input.Password prefix={<LockOutlined style={{ color: '#C6A75E' }} />} placeholder="Minimal 6 karakter" size="large" />
            </Form.Item>
          ...
          <div style={{ textAlign: 'center' }}>
            <Text style={{ fontSize: 13, color: '#595959' }}>
              Sudah punya akun?{' '}
              <Link to="/login" style={{ color: '#1B365D', fontWeight: 600 }}>Masuk</Link>
            </Text>
          </div>
        </Card>
      </div>
